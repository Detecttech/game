...
if (fs.existsSync(schemaPath)) {
  ...
  // Direct fallback to guarantee startup in any packaged container
  db.exec(`
...
try {
  db.exec("ALTER TABLE matches ADD COLUMN timer_seconds INTEGER DEFAULT 30;");
} catch {
  // Column already exists
}

// Automatically seed sample demo teacher, classes, and question banks if database is fresh
try {
  seedDatabase(db);
} catch (err) {
