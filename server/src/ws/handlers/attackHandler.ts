...
import { handleUseAttack } from "../../matchEngine/LiveMatchRegistry";

export function handleAttack(conn: ClientConnection, msg: Envelope) {
  ...
  handleUseAttack(live, playerId, payload?.rewardId ?? "", payload?.targetPlayerId ?? -1, emit);
}
